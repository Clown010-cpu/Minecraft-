...
void spawnEnemies() {
  ...
  for (int i = 0; i < totalEnemies; i++) {
    ...
    float ss;
    if (type == 0) {
      tex = grass; pts = 10; ss = 1.0f;
    } else if (type == 1) {
      tex = birch; pts = 20; ss = 1.2f;
    } else {
      tex = glass; pts = 30; ss = 0.8f;
    }
    targets.add(new Target(x, 0, z, tex, pts, ss));
  }
}
...
void controlCamera() {
  ...
  float dx = cos(udA)*cos(lrA), dy = sin(udA), dz = cos(udA)*sin(lrA);
  focusX = eyeX + dx * 300;
  focusY = eyeY + dy * 300;
  focusZ = eyeZ + dz * 300;
...
void drawHouse() {
  ...
  for (int lvl=1; lvl<=4; lvl++) {
    float yP=oy-lvl*gridSize;
    for (int xi=0; xi<2; xi++) {
      for (int zi=0; zi<2; zi++) {
        pushMatrix();
        translate(ox+xi*5*gridSize, yP, oz+zi*5*gridSize);
    ...
    for (int x=1; x<5; x++) {
      pushMatrix();
      translate(ox + x*gridSize, yP, oz);
      drawBlock(glass, glass, glass);
      popMatrix();
      pushMatrix();
      translate(ox + x*gridSize, yP, oz+5*gridSize);
      drawBlock(glass, glass, glass);
      popMatrix();
    }
    for (int z=1; z<5; z++) {
      pushMatrix();
      translate(ox, yP, oz+z*gridSize);
      drawBlock(glass, glass, glass);
      popMatrix();
      pushMatrix();
      translate(ox+5*gridSize, yP, oz+z*gridSize);
      drawBlock(glass, glass, glass);
      popMatrix();
    }
  }
